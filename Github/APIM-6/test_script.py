...
subprocess.call(["./%s/download_data.sh" % get_target_dir()])
subprocess.call([interpreter_path, "./%s/prediction_train.py" % get_target_dir(),
                 "--output_dir=./checkpoints/%s" % get_target_dir()])
subprocess.call([interpreter_path, "./%s/prediction_train.py" % get_target_dir(),
                 "--output_dir=./checkpoints/%s" % get_target_dir(),
                 "--pretrained_model=./checkpoints/%s/model" % get_target_dir()])
