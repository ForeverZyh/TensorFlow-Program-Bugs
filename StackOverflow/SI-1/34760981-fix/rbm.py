...
class RBM(object):
    ...
    def init_visibles(self, visibles):
        self.visibles = tf.constant(visibles, dtype=tf.float32)

    def cd1(self, learning_rate=0.1):
        h_start = self.propup(self.visibles)
        v_end = self.propdown(h_start)
        h_end = self.propup(v_end)
        w_positive_grad = tf.matmul(tf.transpose(self.visibles), h_start)
        ...
        update_vb = self.v_bias.assign_add(learning_rate * tf.reduce_mean(self.visibles - v_end, 0))
