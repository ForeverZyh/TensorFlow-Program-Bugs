...
for i in range(niter):
    ...
    sess.run(correct_prediction, feed_dict={x: X[:itrain], y_: Y[:itrain]})
    accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
    print(sess.run(accuracy, feed_dict={x: X[:itrain], y_: Y[:itrain]}))

    # EDIT: Calculate test error
    ytest = sess.run(y[itrain:itrain + itest, :], feed_dict={x: X, y_: Y})
    ytest_ = sess.run(y_[itrain:itrain + itest, :], feed_dict={x: X, y_: Y})
    ...
    print(sess.run(test_accuracy, feed_dict={x: X[itrain:itrain + itest, :], y_: Y[itrain:itrain + itest, :]}))
